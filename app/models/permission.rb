class Permission < ActiveRecord::Base
  ...
  def queue_update_auth_table
    unless (previous_changes.keys - ['updated_at']).empty?
      ...
      assets |= Policy.find_by_id(policy_id_was).try(:assets) unless policy_id_was.blank?
  ...
  @@precedence = %w(Person FavouriteGroup WorkGroup Project Institution)
  ...
  def access_type_for(person)
    ...
    if !person.nil? && contributor_type == 'FavouriteGroup'
      ...
      group.nil? ? access_type : group.access_type
    else
      access_type
    end
  end
