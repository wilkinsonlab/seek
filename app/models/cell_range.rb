class CellRange < ActiveRecord::Base
  ...
  def cell_range=(range)
    ...
    unless start_cell.nil? || start_cell.match(/^[a-zA-Z]+[1-9][0-9]*$/).nil?
      ...
      self.start_column = from_alpha(start_cell.sub(/[0-9]+/, ''))
      self.start_row = start_cell.sub(/[A-Z]+/, '').to_i
      self.end_column = nil
      self.end_row = nil
      if end_cell.nil?
        ...
        unless end_cell.match(/^[a-zA-Z]+[1-9][0-9]*$/).nil?
          self.end_column = from_alpha(end_cell.sub(/[0-9]+/, ''))
          self.end_row = end_cell.sub(/[A-Z]+/, '').to_i
        else
          start_row = nil
        end
      end
  ...
  def cell_range
    to_alpha(start_column) + start_row.to_s +
