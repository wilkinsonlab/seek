...
class User < ActiveRecord::Base
  ...
  def self.admin_logged_in?
    self.logged_in_and_registered? && current_user.person.is_admin?
  end

  def self.project_administrator_logged_in?
    self.logged_in_and_registered? && current_user.person.is_project_administrator_of_any_project?
  end

  def self.programme_administrator_logged_in?
    self.logged_in_and_registered? && current_user.person.is_programme_administrator_of_any_programme?
  end

  # programme administrator logged in, but only of activated programmes
  def self.activated_programme_administrator_logged_in?
    self.programme_administrator_logged_in? && current_user.person.administered_programmes.activated.any?
  end
  ...
  def self.asset_housekeeper_logged_in?
    self.logged_in_and_registered? && current_user.person.is_asset_housekeeper?
  end
  ...
  def self.logged_in_and_registered?
    self.logged_in? && current_user.person && current_user.person.id
  end

  def self.logged_in_and_member?
    self.logged_in? && current_user.person.try(:member?)
  end
