class ProjectFolder < ActiveRecord::Base
  ...
  def assets
    project_folder_assets.collect(&:asset)
  end
  ...
  def self.initialize_default_folders(project, yaml_path = File.join(Rails.root, 'config', 'default_data', 'default_project_folders.yml'))
    fail Exception.new("This #{I18n.t('project')} already has folders defined") unless ProjectFolder.root_folders(project).empty?
