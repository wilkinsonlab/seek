...
class Workflow < ActiveRecord::Base
  ...
  has_many :input_ports, class_name: 'WorkflowInputPort',
                         conditions: proc { "workflow_version = #{version}" },
                         dependent: :destroy
  ...
  explicit_versioning(version_column: 'version') do
    ...
    has_one :content_blob, primary_key: :workflow_id, foreign_key: :asset_id, conditions: proc { ['content_blobs.asset_version =? AND content_blobs.asset_type =?', version, parent.class.name] }
    has_many :input_ports, class_name: 'WorkflowInputPort',
  ...
  def collect_related_items
    ...
    related_types.each do |type|
      ...
      related[type][:items] = send method_name
      if method_name == 'runs'
