class FavouriteGroup < ActiveRecord::Base
  ...
  def is_whitelist_or_blacklist?
    [FavouriteGroup::BLACKLIST_NAME, FavouriteGroup::WHITELIST_NAME].include?(name)
  end
  ...
  def self.get_all_without_blacklists_and_whitelists(user_id)
    ...
    all_groups.collect { |g| [g.name, g.id] unless [FavouriteGroup::WHITELIST_NAME, FavouriteGroup::BLACKLIST_NAME].include?(g.name) }.compact
  end
end
